...
- Static frontend: `haema-quiz/` in `jochim-uke/medical-search-tools`; linked from the shared homepage.
...
2. Store the edition as `haema-quiz/content/YYYY-Www.json` on the default branch of `jochim-uke/medical-search-tools` using the GitHub connector. Use existing file contents on a retry; do not silently replace an edition.
...
`node --check haema-quiz/app.js`. Validate the API's public reads, hidden answer fields, answer grading, rejected anonymous deletion, rejected wrong password, successful admin login, deletion of a synthetic test question, and logout/session revocation. Never delete a real question merely for a test.

Canonical app: https://jochim-uke.github.io/medical-search-tools/haema-quiz/
